...
describe('No Dupe Scenarios', () => {
  it('returns errors if the rule fails', async () => {
    const featureDirectory = path.join(import.meta.dirname, './features/no-dupe-scenarios')
    ...
    expect(errors[0].message).to.eq(
      `Found duplicate scenario "This is duplicated" in "invalid2.feature, invalid1.feature"`,
    )
  })
})
